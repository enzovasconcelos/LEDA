...
public class RepositorioProduto<T extends Produto> {
	...
	private int procurarIndice(int codigo) {
	        for(int atual = 0; atual < this.index + 1; atual++) {
                    T produtoAtual = (T) this.produtos[atual];
	...
	public void remover(int codigo) {
		int indice = procurarIndice(codigo);	
                for(int atual = indice; atual < this.index; atual++) {
                    this.produtos[atual] = this.produtos[atual + 1];
                }
